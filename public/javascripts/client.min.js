...
'<span class="sh_comment">/**$1</span>').replace(/\*(.*)/g,'<span class="sh_comment">* $1</span>').replace(/(\d{2,4}?)/g,'<span class="sh_number">$1</span>')};render.prototype.higlightBash=function(){this.cache=this.cache.replace(/(ls|cd|tree|rm|\sexpress\s|npm|\snode\s)/g,"<span class=sh_keyword>$1</span>").replace(/(create)/g,"<span class=sh_number>$1</span>").replace(/(Jxck\$)/g,"<span class=sh_function>$1</span>").replace(/(\d{2,4}?)/g,'<span class="sh_number">$1</span>')};
...
socket.on("empty2",function(a){empty2Render.htmlRender(a)});socket.on("disconnect",function(){log("disconnected")})});$(function(){var a=$("section").height(),b=$("h2").height();$("iframe").height(a);$("pre.sh_javascript, pre.sh_sh").height(a-b*2);$("pre.half").height(a/2.7);$("pre.full").height(a-b/2);$("#app").height(a*0.7);$("#routes").height(a*0.2)});
