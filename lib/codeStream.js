...
CodeStream.prototype.readCode = function() {
  fs.readFile(this.file, 'utf8', function(err, data) {
    ...
    if (data !== this.cache) {
      this.emit('code', data);
      var patch = make_patch(this.cache, data);
...
var app = '../sample/app.js',
    socketserver = '../sample/socketserver.js',
...
// start to read files
appStream.readCode();
socketserverStream.readCode();
clientStream.readCode();
layoutStream.readCode();
indexStream.readCode();
...
socket.on('connect', function() {
  ...
  socketserverStream.on('patch', function(patch) {
    socket.emit('socketserver', patch);
  });

  clientStream.on('patch', function(data) {
